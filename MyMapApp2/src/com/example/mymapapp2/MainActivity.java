...
import com.example.mymapapp2.R;
...
public class MainActivity extends FragmentActivity implements
		OnCameraChangeListener, OnMarkerDragListener, OnMarkerClickListener,
		OnMapClickListener {
	...
	private float dens = Resources.getSystem().getDisplayMetrics().density;
	...
	private void setUpMapIfNeeded() {
		if (mMap == null || mMap2 == null) {
			...
			mMap2.moveCamera(CameraUpdateFactory.newLatLngZoom(latLng, 17));
			// /////////////////////////////////////////////////////
		}
		if (mMap != null && mMap2 != null)
			setUpMap();
	...
	private void setUpMap() {
		...
		if (poly == null)
			poly = mMap.addPolygon(new PolygonOptions().add(new LatLng(0, 0))
					.strokeWidth(2).strokeColor(Color.BLACK)
	...
	public void mvScreen() {
		...
		rl.setX(mMap.getProjection().toScreenLocation(
				mkr.get(selmkr - 1).getPosition()).x
				- 78 * dens);
		rl.setY(mMap.getProjection().toScreenLocation(
				mkr.get(selmkr - 1).getPosition()).y
				- 170 * dens);
	}
